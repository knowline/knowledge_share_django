...
class IndexPageTests(TestCase):
    ...
    def test_index_contains_hello_message(self):
        # Check if there is the message '在信息时代，知道知识在哪里比知道知识是什么更重要'
        # Chapter 4
        response = self.client.get(reverse('index'))
        self.assertIn(b'在信息时代，知道知识在哪里比知道知识是什么更重要', response.content)
...
class Chapter4ViewTests(TestCase):
    def test_index_contains_hello_message(self):
        ...
        response = self.client.get(reverse('index'))
        self.assertIn('在信息时代，知道知识在哪里比知道知识是什么更重要', response.content)
